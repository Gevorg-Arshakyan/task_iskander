  ...
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
      <div
        ...
        :key="product.id"
        class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow cursor-pointer group"
      >
        <!-- Product Image -->
        <div class="aspect-square bg-[#7EC4E8] relative">
          <img :src="product.image" :alt="product.name" class="w-full h-full object-cover" />

          <!-- Heart Icon (for suggestions) -->
          <button
          ...
          >
            <svg
              ...
              :class="
                product.liked ? 'text-red-500 fill-red-500' : 'text-gray-400 hover:text-red-500'
              "
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
          ...
          <!-- Checkbox Icon (for kits) -->
          <div
            v-if="type === 'kits'"
            class="absolute top-3 right-3 p-2 bg-white rounded-full shadow-sm"
          >
            <svg
              class="w-5 h-5"
              :class="product.selected ? 'text-[#4A8FB9] fill-[#4A8FB9]' : 'text-gray-400'"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center border border-gray-300 rounded-lg">
              ...
              <button
                ...
                :class="
                  product.quantity === 0
                    ? 'bg-[#4A8FB9] text-white hover:bg-blue-700 rounded-full'
                    : ''
                "
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
